...
# a = base64.b64encode(bytes.fromhex(ct)).decode()
# print (a)
...
#     s = 0
#     data = data.decode('utf-8', errors='ignore').lower()
...
#     return any(chunks.count(ch) > 1 for ch in chunks)

# URL = 'https://cryptopals.com/static/challenge-data/8.txt'
# ciphertexts = requests.get(URL).text.split("\n")

# for num, ciphertext in enumerate(ciphertexts, start=1):
#     if detect_aes_ecb(ciphertext):
#         print(f"AES-ECB detected in line {num}: {ciphertext}")
